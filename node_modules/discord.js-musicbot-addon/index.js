 ...
 */     if (musicbot.cooldown.exclude.includes("skip")) console.warn(`[MUSIC] Excluding SKIP CMD from cooldowns can cause issues.`);
    ...
    musicbot.playFunction = (msg, suffix, args) => {
      ...
      if (q.songs.length >= musicbot.maxQueueSize && musicbot.maxQueueSize !== 0) return msg.channel.send(musicbot.note('fail', 'Достигнут максимальный лимит очереди!'));
      ...
      if (searchstring.startsWith('http') && searchstring.includes("list=")) {
        msg.channel.send(musicbot.note("search", `Ищем видео~`));
        ...
        ytpl(playid, function(err, playlist) {
          ...
          if (playlist.items.length <= 0) return msg.channel.send(musicbot.note('fail', `Не удалось получить видео`));
          if (playlist.total_items >= 1000) return msg.channel.send(musicbot.note('fail', `Больше нельзя добовлять видео в лейлист. Максимальное количество песен - 100`));
          var index = 0;
          var ran = 0;
          ...
          playlist.items.forEach(video => {
            ...
            if (ran >= playlist.items.length) {
              if (index == 0) msg.channel.send(musicbot.note('fail', `Неудалось получит ниодного видео из плейлиста!`))
